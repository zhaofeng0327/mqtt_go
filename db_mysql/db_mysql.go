...
type MqttMsg struct {
	Id                                       int64
	Timestamp, Deviceid, Topic, Cmd, Msgdata string
}

const DATABASE_NAME = "anker_qa"
...
func createTable(conn *sql.DB, database_name, table_name string) error {
	...
	createTableStatements[2] = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %v %v", table_name,
		"(id INT UNSIGNED NOT NULL AUTO_INCREMENT, timestamp TEXT NULL, deviceid TEXT NULL, topic TEXT NULL, cmd TEXT NULL, msgdata TEXT NULL, PRIMARY KEY (id))")
...
func scanMqttMsg(s rowScanner) (*MqttMsg, error) {
	var (
		...
		cmd       sql.NullString
		msgdata   sql.NullString
	)

	if err := s.Scan(&id, &timestamp, &deviceid, &topic, &cmd, &msgdata); err != nil {
		return nil, err
	}
...
func (db *MysqlDB) InsertMqttMsg(table_name string, msg *MqttMsg) (id int64, err error) {
	stmt, err := db.conn.Prepare("INSERT INTO " + table_name + " (timestamp, deviceid, topic, cmd, msgdata) VALUES ( ?, ?, ?, ?, ?)")
	if err != nil {
		return 0, fmt.Errorf("mysql: prepare list by cmd: %v", err)
	}

	r, err := execAffectingOneRow(stmt, msg.Timestamp, msg.Deviceid, msg.Topic, msg.Cmd, msg.Msgdata)
	if err != nil {
		return 0, err
	}
...
func (db *MysqlDB) UpdateMqttMsg(table_name string, msg *MqttMsg) error {
	...
	stmt, err := db.conn.Prepare("UPDATE " + table_name + " SET id=?, timestamp=?, deviceid=?, topic=?, cmd=?, msgdata=?, WHERE id = ?")
	_, err = execAffectingOneRow(stmt, msg.Id, msg.Timestamp, msg.Deviceid, msg.Topic, msg.Cmd, msg.Msgdata)
	return err
}
